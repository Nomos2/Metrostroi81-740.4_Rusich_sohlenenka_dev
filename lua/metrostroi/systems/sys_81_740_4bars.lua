...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_740_4BARS")
...
function TRAIN_SYSTEM:Initialize()
    ...
    self.Braking = false
    self.LN = 0 -- ЛН 
	self.DAU = 0 -- ДАУ как 1/5, но симулирует ДАУ
	self.AO = 0 -- АО
	self.Dnepr = 0  -- Днепр 1/5 и 1/6
	self.Ispr = 0
    self.PN1 = 0 
    self.PN2 = 0
    self.BTB = 0
...
function TRAIN_SYSTEM:Think(dT)
    ...
    local Emer = Active and Train.KV["KRR15-16"] > 0
    ...
    if EnableALS then
        ...
        if TwoToSix and self.F6 then
            self.LN = true
		    self.AO = false
		elseif TwoToSix and not self.LN and self.KB then
		    self.SpeedLimit = 20
		    self.NextLimit = 20
        end
	else
    ...
    if Active then
        ...
        if TwoToSix and (not self.LN or self.NextLimit == math.max(20,SpeedLimit) and self.F6 == 0) then
            SpeedLimit = math.min(20,SpeedLimit)
        end
		if self.DAU then
			self.NextLimit = 0
		end
        ...
        --Противоскат
        if self.Speed < 0.1 and self.PN2 == 0 and (BUPKMState <= 0 or self.Drive == 0) then
            self.PN2 = 1
            self.Starting = nil
        end
		...
		--[[self.AO = TwoToSix and self.RealF5
		if self.AO and (self.KB or not self.KB)then
	        self.Drive = 0
			self.DisableDrive = 0
		end]]
    else
